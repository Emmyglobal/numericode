...
export default function AdminCourseBuilderPage() {
  ...
  const invalidate = () => queryClient.invalidateQueries({ queryKey: ['admin', 'course-builder', courseId] })
  ...
  const updateModuleMutation = useMutation({
    mutationFn: ({ moduleId, title }: { moduleId: string; title: string }) => api.put(`/admin/modules/${moduleId}`, { title }),
  ...
  const deleteModuleMutation = useMutation({
    mutationFn: (moduleId: string) => api.delete(`/admin/modules/${moduleId}`),
  ...
  const createLessonMutation = useMutation({
    mutationFn: ({ moduleId, title, content }: { moduleId: string; title: string; content?: string }) =>
      api.post(`/admin/modules/${moduleId}/lessons`, { title, content }),
  ...
  const updateLessonMutation = useMutation({
    mutationFn: ({ lessonId, title, content }: { lessonId: string; title?: string; content?: string }) =>
      api.put(`/admin/lessons/${lessonId}`, { title, content }),
  ...
  const createQuizMutation = useMutation({
    ...
    }) => api.post(`/admin/lessons/${lessonId}/quiz`, { title, description, passingScore, timeLimit, questions }),
  ...
  const updateQuizMutation = useMutation({
    ...
    }) => api.put(`/admin/quizzes/${quizId}`, { title, description, passingScore, timeLimit }),
  ...
  const deleteQuizMutation = useMutation({
    mutationFn: (quizId: string) => api.delete(`/admin/quizzes/${quizId}`),
  ...
  const createAssignmentMutation = useMutation({
    ...
    }) => api.post(`/admin/lessons/${lessonId}/assignment`, { title, description, dueDate, totalMarks, passingScore }),
  ...
  const updateAssignmentMutation = useMutation({
    ...
    }) => api.put(`/admin/assignments/${assignmentId}`, { title, description, dueDate, totalMarks, passingScore }),
  ...
  const deleteAssignmentMutation = useMutation({
    mutationFn: (assignmentId: string) => api.delete(`/admin/assignments/${assignmentId}`),
  ...
  const handleEditQuiz = async (quizId: string) => {
    try {
      const r = await api.get<{ data: QuizDetail }>(`/admin/quizzes/${quizId}`)
  ...
  const handleEditAssignment = async (assignmentId: string) => {
    try {
      const r = await api.get<{ data: AssignmentDetail }>(`/admin/assignments/${assignmentId}`)
